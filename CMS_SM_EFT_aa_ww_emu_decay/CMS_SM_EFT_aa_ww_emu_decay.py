...
def save_plot_and_csv(x, y_sm, y_eft, edges, out_prefix: str, xlabel: str, logy: bool, sm_label: str, eft_label: str):
    plt.figure()
    plt.step(edges[:-1], y_sm, where="post", label=sm_label, linewidth=1.8)
    plt.step(edges[:-1], y_eft, where="post", label=eft_label, linewidth=1.8)
    plt.xlabel(xlabel)
...
def analyze_one_sample(path: str, bins_e_mu=50, rng_e_mu=(0.0, 500.0),
...
def main():
    ap = argparse.ArgumentParser(description="Compare SM vs EFT pT spectra from LHE files (aa->WW->eμ).")
    ...
    save_plot_and_csv(x_mu, y_mu_sm, y_mu_eft, muedges, "pt_mu_compare_SM_vs_EFT",  r"$p_T(\mu^\pm)$ [GeV]", args.logy, label(res_sm["meta"], "SM"), label(res_eft["meta"], "EFT"))
    save_plot_and_csv(x_em, y_em_sm, y_em_eft, emedges, "pt_emu_compare_SM_vs_EFT", r"$p_T(e\mu)$ [GeV]",    args.logy, label(res_sm["meta"], "SM"), label(res_eft["meta"], "EFT"))
