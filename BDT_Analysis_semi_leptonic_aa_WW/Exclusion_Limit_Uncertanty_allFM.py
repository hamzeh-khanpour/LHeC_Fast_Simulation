...
sigma_SM    = 1.538327e+01  # Standard Model cross-section

sigma_FM0_1 = 0.022850099999999998 * 1000.0    # Cross-section for FM0 = 10^-8
...
# Given parameters
luminosity = 100  # in fb^-1


# FM0 value from anoinputs file in MG
FM0_value = 100 * 1e-12 # The input value for FM0 in the cross-section calculation in MG.
...
signal_efficiency = 0.1405  # Efficiency for signal as decimal
background_efficiency = 0.1205  # Efficiency for background as decimal
...
# Compute expected signal events for two-sided range of F_M0
if F_i_limit > 0:
    F_i_values = np.linspace(-2 * F_i_limit, 2 * F_i_limit, 1000)  # Cover both negative and positive
else:
    F_i_values = np.linspace(-1000e-12, 1000e-12, 1000)  # Small range to avoid flat plots
...
# Updated plot with negative and positive range
...
# Two-sided exclusion limits
plt.axvline(F_i_upper, color='g', linestyle='--', label=r'Upper $(f_{M_0} / \Lambda^4)^{\text{lim}}$', linewidth=3)
plt.axvline(F_i_lower, color='g', linestyle='--', label=r'Lower $(f_{M_0} / \Lambda^4)^{\text{lim}}$', linewidth=3)
