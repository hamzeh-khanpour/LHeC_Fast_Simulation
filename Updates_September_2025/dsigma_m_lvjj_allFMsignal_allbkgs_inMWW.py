...
#    "$FM_{1} / \Lambda^4$": "aa_ww_semi_leptonic_NP_1_FM1_Delphes_Pythia.root",
    "FM2_Lambda4": "aa_ww_semi_leptonic_NP_1_FM2_Delphes_Pythia_10.root",
...
signal_cross_sections = {
    ...
    "FM2_Lambda4": 0.0153508, # 0.022850099999999998,             # pb
...
aa_tautau_background_cross_section   = 1.806765e-01  # pb  Laurent elastic

aa_tautau_inel_background_cross_section = 1.156165e-01  # pb  Laurent inelastic
...
hist_m_lvjj_aa_ww = ROOT.TH1F("hist_m_lvjj_aa_ww", "m(lνjj) (aa_ww); m_{lνjj} [GeV]; Entries", num_bins, *m_lvjj_range)
...
hist_m_lvjj_aa_ttbar = ROOT.TH1F("hist_m_lvjj_aa_ttbar", "m(lνjj) (aa_ttbar); m_{lνjj} [GeV]; Entries", num_bins, *m_lvjj_range)
...
hist_m_lvjj_aa_tautau = ROOT.TH1F("hist_m_lvjj_aa_tautau", "m(lνjj) (aa_tautau); m_{lνjj} [GeV]; Entries", num_bins, *m_lvjj_range)
...
hist_m_lvjj_aa_tautau_inel = ROOT.TH1F("hist_m_lvjj_aa_tautau_inel", "m(lνjj) (aa_tautau_inel); m_{lνjj} [GeV]; Entries", num_bins, *m_lvjj_range)
...
hist_m_lvjj_inclusive_ttbar                = ROOT.TH1F("hist_m_lvjj_inclusive_ttbar",                "m(lνjj) (inclusive_ttbar); m_{lνjj} [GeV]; Entries", num_bins, *m_lvjj_range)
...
hist_m_lvjj_single_top                = ROOT.TH1F("hist_m_lvjj_single_top",                "m(lνjj) (single_top); m_{lνjj} [GeV]; Entries", num_bins, *m_lvjj_range)
...
hist_m_lvjj_w_production                = ROOT.TH1F("hist_m_lvjj_w_production",                "m(lνjj) (w_production); m_{lνjj} [GeV]; Entries", num_bins, *m_lvjj_range)
...
hist_m_lvjj_z_production                = ROOT.TH1F("hist_m_lvjj_z_production",                "m(lνjj) (z_production); m_{lνjj} [GeV]; Entries", num_bins, *m_lvjj_range)
...
hist_m_lvjj_wwj_production                = ROOT.TH1F("hist_m_lvjj_wwj_production",                "m(lνjj) (wwj_production); m_{lνjj} [GeV]; Entries", num_bins, *m_lvjj_range)
...
hist_m_lvjj_zzj_production                = ROOT.TH1F("hist_m_lvjj_zzj_production",                "m(lνjj) (zzj_production); m_{lνjj} [GeV]; Entries", num_bins, *m_lvjj_range)
...
hist_m_lvjj_wzj_production                = ROOT.TH1F("hist_m_lvjj_wzj_production",                "m(lνjj) (wzj_production); m_{lνjj} [GeV]; Entries", num_bins, *m_lvjj_range)
...
def _infer_edges_from_centers_or_leftedges(x):
    """Given N values (centers or left edges), infer N+1 edges.
    - If x are left edges and uniform, we use the provided global `bin_width_m_lvjj` if it exists,
      else infer from the median spacing.
    - If x look like bin centers, edges are built by midpoints between consecutive centers.
    """
    x = np.asarray(x, dtype=float)
    dx = np.diff(x)
    if np.all(dx > 0):
        # Heuristic: if gaps are roughly uniform and we have a global width, assume left edges
        try:
            bw = float(globals().get("bin_width_m_lvjj", None))
        except Exception:
            bw = None
        if bw is None:
            # Decide center vs left-edge by comparing typical spacing and local symmetry
            # If centers: neighbors should be ~symmetric, so we'll construct edges via midpoints
            # If left-edges and uniform: we can treat x as left edges with width=median(dx)
            med = np.median(dx)
            # Build two candidate first edges assuming centers vs left-edges
            # We'll prefer "centers" if the first bin would start before zero by half spacing AND
            # the histogram was likely built by centers.
            # In practice, both choices integrate to nearly the same area; pick centers as default.
            # -> Use centers
            edges_mid = np.r_[x[0]-0.5*dx[0], 0.5*(x[:-1]+x[1:]), x[-1]+0.5*dx[-1]]
            return edges_mid
        else:
            # Treat as left edges with known width
            return np.r_[x, x[-1]+bw]
    raise ValueError("Bins are not strictly increasing; cannot infer edges.")




def integrate_tail(bin_axis, y, thr):
    """
    Integrate a histogram tail:
      bin_axis: array of length N+1 (edges) OR length N (centers/left-edges)
      y:        array of length N with dσ/dW in pb/GeV
      thr:      threshold in GeV
    Returns σ(W>thr) in pb.
    """
    bin_axis = np.asarray(bin_axis, dtype=float)
    y = np.asarray(y, dtype=float)

    # If axis length doesn't match N+1, infer edges
    if bin_axis.shape[0] == y.shape[0] + 1:
        edges = bin_axis
    elif bin_axis.shape[0] == y.shape[0]:
        edges = _infer_edges_from_centers_or_leftedges(bin_axis)
    else:
        raise ValueError("Incompatible bin axis and histogram lengths.")
...
for signal_name, dsigma_data in signal_dsigma.items():
    ...
    plt.step(mlvjj_bins, dsigma, where="mid", alpha=0.7,
             label=f"Signal ($W^+W^-$) [$f_{{M2}} / \\Lambda^4$]",
...
#plt.xlabel(r"$m(\ell \nu jj) \ \mathrm{[GeV]}$")
plt.xlabel(r"$W_{\gamma \gamma} \ \mathrm{[GeV]}$")
#plt.ylabel(r"$\frac{d\sigma}{dm(\ell \nu jj)} \ \mathrm{[pb/GeV]}$")
plt.ylabel(r"$\frac{d\sigma}{dW_{\gamma \gamma}} \ \mathrm{[pb/GeV]}$")
...
plt.tight_layout()
plt.savefig("dsigma_m_lvjj_allFMsignal_allbkgs_inMWW.pdf", dpi=600)
...
plt.grid(True, linestyle="--", alpha=0.6)
plt.tight_layout()
plt.savefig("/normalized_dsigma_m_lvjj_allFMsignal_allbkgs_inMWW.pdf", dpi=600)
...
output_file = ROOT.TFile("/home/hamzeh-khanpour/Delphes-3.5.0/output_histograms_FM2.root", "RECREATE")
