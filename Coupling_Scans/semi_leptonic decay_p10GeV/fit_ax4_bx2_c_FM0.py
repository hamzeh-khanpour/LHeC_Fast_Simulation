...
# Load data from file
filename = "cross_sections_FM0.txt"
fm2_vals = []
xsec_vals = []

with open(filename, 'r') as file:
    for line in file:
        if line.startswith("FM0") or not line.strip():
            continue
...
# Labels and style
plt.xlabel('$f_{M0}/\Lambda^4$ Value (scaled by $10^{-12}$)')
plt.ylabel('Cross-section (pb)')
plt.title('Cross-section vs $f_{M0}/\Lambda^4$ (Fit)')
plt.grid(True)
plt.legend()
plt.tight_layout()

# Save plot as PDF
plt.savefig("fit_cross_section_vs_FM0.pdf", format='pdf')
