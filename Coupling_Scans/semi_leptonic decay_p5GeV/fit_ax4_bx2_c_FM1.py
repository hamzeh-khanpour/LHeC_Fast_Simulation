...
# Load data from file
filename = "cross_sections_FM1.txt"
fm2_vals = []
xsec_vals = []

with open(filename, 'r') as file:
    for line in file:
        if line.startswith("FM1") or not line.strip():
            continue
...
# Create fit curve
x_fit = np.linspace(-2100, 2100, 500)
...
# Labels and style
plt.xlabel('$f_{M1}/\Lambda^4$ Value (scaled by $10^{-12}$)')
plt.ylabel('Cross-section (fb)')
plt.title('Cross-section vs $f_{M1}/\Lambda^4$ (Fit)')
plt.grid(True)
plt.legend()
plt.tight_layout()

# Save plot as PDF
plt.savefig("fit_cross_section_vs_FM1.pdf", format='pdf')
